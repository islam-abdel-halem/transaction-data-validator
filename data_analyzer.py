...
def main() -> None:
    """Run the Phase 4 analysis workflow and print concise summaries."""
    dataset_path = locate_dataset()
    sales_dataframe = load_sales_data(dataset_path)
    analyzed_dataframe, city_summary, monthly_summary = transform_sales_data(sales_dataframe)

    output_path = Path(__file__).resolve().parent / "analyzed_sales.csv"
    save_analyzed_data(analyzed_dataframe, output_path)

    print(f"Loaded dataset: {dataset_path}")
    print(f"Rows analyzed: {len(analyzed_dataframe)}")
    print(f"Output saved to: {output_path}")
    print("\nTotal Sales Per City (USD):")
